...
_PLATFORMS = [
    ...
    },
    {
        ...
        "label": "Zalo Bot",
        "emoji": "💬",
        "token_var": "ZALO_BOT_TOKEN",
        "setup_instructions": [
            "1. Open Zalo and search for the OA Zalo Bot Manager",
            "2. Use Create bot (Zalo Bot Creator) — bot name must start with the Bot prefix",
            "3. Copy the Bot Token sent to your Zalo account after creation",
            "4. Default: polling (getUpdates). If you used a webhook before, call deleteWebhook or use webhook mode below.",
            "5. Production: set ZALO_ALLOWED_USERS (comma-separated). Optional: ZALO_HOME_CHANNEL for cron delivery.",
            "6. Webhook mode: public https URL (ngrok/reverse proxy), ZALO_WEBHOOK_SECRET (8–256 chars), pip install aiohttp",
            "Docs: https://bot.zapps.me/docs/ — Polling guide: https://bot.zaloplatforms.com/docs/build-your-bot/",
        ],
        "vars": [
            {"name": "ZALO_BOT_TOKEN", "prompt": "Zalo Bot token", "password": True,
             "help": "Token from Zalo Bot Creator (sent via Zalo message after you create the bot)."},
            {"name": "ZALO_ALLOWED_USERS", "prompt": "Allowed user IDs (comma-separated, or empty)", "password": False,
             "is_allowlist": True,
             "help": "Restrict which Zalo users can message the bot."},
            {"name": "ZALO_HOME_CHANNEL", "prompt": "Home chat ID (optional, for cron/notifications)", "password": False,
             "help": "Zalo chat id for scheduled results and notifications."},
            {"name": "ZALO_CONNECTION_MODE", "prompt": "polling or webhook (default: polling)", "password": False,
             "help": "polling = getUpdates long polling. webhook = local HTTP server + setWebhook."},
            {"name": "ZALO_WEBHOOK_PUBLIC_URL", "prompt": "Public https webhook URL (webhook mode only)", "password": False,
             "help": "Full URL passed to setWebhook; must reach your gateway (e.g. https://tunnel.example/zalo/webhook)."},
            {"name": "ZALO_WEBHOOK_SECRET", "prompt": "Webhook secret 8–256 chars (webhook mode)", "password": True,
             "help": "Zalo sends this in header X-Bot-Api-Secret-Token; must match setWebhook."},
        ],
    },
    {
        "key": "feishu",
